...
func setupMinimizeToTray(m *Manager) error {
	...
	// Note: ret == 0 might be valid, so we don't error check here
	// If there's a real error, SetWindowLongPtr will fail
	originalWndProc = ret
	...
	// Check if SetWindowLongPtr failed (ret == 0 and GetLastError indicates failure)
	if ret == 0 {
